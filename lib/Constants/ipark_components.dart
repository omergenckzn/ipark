...
class DatePickerTextButtonTime extends StatefulWidget {
  final DateTime initialDate;
  final Function(DateTime date) setDate;
  const DatePickerTextButtonTime(
      {Key? key, required this.initialDate, required this.setDate})
      : super(key: key);

  @override
  State<DatePickerTextButtonTime> createState() => _DatePickerTextButtonTimeState();
}

class _DatePickerTextButtonTimeState extends State<DatePickerTextButtonTime> {
  bool isDatePickerOpen = false;
  late DateTime pickedDate;
  ...
  Widget build(BuildContext context) {
    ...
    String todayText = "Today - ${DateFormat("Hm").format(pickedDate)}";
    bool isToday = isTodayDecider();
    String pickedDateFormattedString =
        "${pickedDate.day}.${pickedDate.month}.${pickedDate.year} - ${DateFormat("Hm").format(pickedDate)}";
